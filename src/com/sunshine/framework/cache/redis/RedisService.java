...
public interface RedisService {
	 ...
	 * @return
	 */
	public <T> Boolean pipelinedHash(String cacheKey, Map<String, List<T>> dataMap) throws SystemException;

	/**
	 * @Description 管道写入数据并清除之前的数据(hash结构)
	 * @param cacheKey
	 * @param addDatas
	 * @return
	 * @date 2017年11月16日
	 */
	public Boolean pipelineHashDataWithDelOld(String cacheKey, Map<String, String> addDatas);
	...
	/**
	 * 如果key存在，并且是String类型，则在其value值后附加上传入的value值。 如果key不存在则创建该key并设置为空字符后附加上value的值
	 * 
	 * @param key
	 * @param value
	...
	/**
	 * 指定Key关联的链表中,删除前count个值等于value的元素. 如果count大于0,从头向尾遍历并删除 如果count小于0，则从尾向头遍历并删除. 如果count等于0，则删除链表中所有等于value的元素。 如果指定的Key不存在，则直接返回0
	 * 
	 * @param key
	 * @param count
	 * @param value
	 ...
	 * @param member
	 *            源集合中的成员
	 * @return 状态码,1成功,0失败 注:只支持单机,不支持切片
	 */
	 ...
	 * 
	 * @param keys
	 * @return 合并后的结果集合 注：分布式redis不支持,只支持单机
	 */
	 ...
	 * 
	 * @param keys
	 * @return 交集成员的集合 注：分布式redis不支持,只支持单机
	 */
	 ...
	 * @param keys
	 *            比较的集合
	 * @return 新集合中的记录数 注：分布式redis不支持,只支持单机
	 */
	 ...
	 * @param key
	 * @param member
	 * @return 存在返回1,不存在返回0 注：分布式redis不支持,只支持单机
	 */
