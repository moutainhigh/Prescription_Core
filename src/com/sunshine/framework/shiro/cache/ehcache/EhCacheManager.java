 ...
 * under the License.
 */
package com.sunshine.framework.shiro.cache.ehcache;
...
import com.sunshine.framework.shiro.cache.ehcache.EhCache;
import com.sunshine.framework.shiro.cache.ehcache.EhCacheManager;

/**
 * Shiro {@code CacheManager} implementation utilizing the Ehcache framework for all cache functionality.
 * <p/>
 * This class can {@link #setCacheManager(net.sf.ehcache.CacheManager) accept} a manually configured {@link net.sf.ehcache.CacheManager
 * net.sf.ehcache.CacheManager} instance, or an {@code ehcache.xml} path location can be specified instead and one will be constructed. If neither are
...
public class EhCacheManager implements CacheManager, Initializable, Destroyable {
	...
	/**
	 * Indicates if the CacheManager instance was implicitly/automatically created by this instance, indicating that it should be automatically
	 ...
	 *
	 * @param manager
	 *            the wrapped Ehcache {@link net.sf.ehcache.CacheManager CacheManager} instance.
	 */
	...
	/**
	 * Returns the resource location of the config file used to initialize a new EhCache CacheManager instance. The string can be any resource path
	 * supported by the {@link org.apache.shiro.io.ResourceUtils#getInputStreamForPath(String)} call.
	 * <p/>
	 * This property is ignored if the CacheManager instance is injected directly - that is, it is only used to lazily create a CacheManager if one is
	 * not already provided.
	 *
	 * @return the resource location of the config file used to initialize the wrapped EhCache CacheManager instance.
	 */
	...
	/**
	 * Sets the resource location of the config file used to initialize the wrapped EhCache CacheManager instance. The string can be any resource path
	 * supported by the {@link org.apache.shiro.io.ResourceUtils#getInputStreamForPath(String)} call.
	 * <p/>
	 * This property is ignored if the CacheManager instance is injected directly - that is, it is only used to lazily create a CacheManager if one is
	 * not already provided.
	 ...
	 *            resource location of the config file used to create the wrapped EhCache CacheManager instance.
	 */
	...
	/**
	 * Acquires the InputStream for the ehcache configuration file using {@link ResourceUtils#getInputStreamForPath(String)
	 * ResourceUtils.getInputStreamForPath} with the path returned from {@link #getCacheManagerConfigFile() getCacheManagerConfigFile()}.
	 *
	 ...
	 * <p/>
	 * If a {@link #setCacheManager CacheManager} has been explicitly set (e.g. via Dependency Injection or programatically) prior to calling this
	 * method, this method does nothing.
	 * <p/>
	 * However, if no {@code CacheManager} has been set, the default Ehcache singleton will be initialized, where Ehcache will look for an
	 * {@code ehcache.xml} file at the root of the classpath. If one is not found, Ehcache will use its own failsafe configuration file.
	 * <p/>
	 * Because Shiro cannot use the failsafe defaults (fail-safe expunges cached objects after 2 minutes, something not desirable for Shiro sessions),
	 ...
	 * <p/>
	 * If another component injected a non-null CacheManager into this instace before calling {@link #init() init}, this instance expects that same
