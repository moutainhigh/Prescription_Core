...
public class LogbackContainer implements Container {
	...
	public static String LOGBACK_FILE_PATH = "logback/logback.xml";
	...
	public void start() {
		try {
			...
			org.slf4j.LoggerFactory.getLogger(LogbackContainer.class).info("using logger: logback");
		...
		/*
		 * String file = ConfigUtils.getProperty(LOGBACK_FILE); if (file != null && file.length() > 0) { String level =
		 * ConfigUtils.getProperty(LOGBACK_LEVEL); if (level == null || level.length() == 0) { level = DEFAULT_LOGBACK_LEVEL; } // maxHistory=0
		 * Infinite history int maxHistory = StringUtils.parseInteger(ConfigUtils.getProperty(LOGBACK_MAX_HISTORY));
		 * 
